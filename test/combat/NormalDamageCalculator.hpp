...
TEST_CASE("Test NormalDamageCalculator - damage over distance drop off", "[require]") {
  ...
  float damage_at_0 = Normal(unit_a, unit_b, terrain).calculate_damage_over_distance();

  terrain = src::combat::Terrain{0, 50, src::unit::CamoType::Normal}; // 0 cover, max range = 50% of effective range
  float damage_at_50 = Normal(unit_a, unit_b, terrain).calculate_damage_over_distance();

  terrain = src::combat::Terrain{0, 100, src::unit::CamoType::Normal}; // 0 cover, max range = effective range
  float damage_at_100 = Normal(unit_a, unit_b, terrain).calculate_damage_over_distance();

  terrain = src::combat::Terrain{0, 200, src::unit::CamoType::Normal}; // 0 cover, max range = twice effective range
  float damage_at_200 = Normal(unit_a, unit_b, terrain).calculate_damage_over_distance();
...
TEST_CASE("Test NormalDamageCalculator - camo - no effect from no camo", "[require]") {
  auto unit_a = UnitFactory::standard(0, 0), unit_b = UnitFactory::standard(1, 1);
  src::combat::Terrain terrain{0, 0, src::unit::CamoType::Normal}; // 0 cover, 0 max range, normal landscape
  std::dynamic_pointer_cast<src::unit::Unit>(unit_b)->set_camo(src::unit::CamoType::Normal);

  float theoretical_damage = Normal(unit_a, unit_b, terrain).calculate_damage_over_distance();
  float damage_including_camo = Normal(unit_a, unit_b, terrain).calculate_damage_through_camo(theoretical_damage);

  REQUIRE(theoretical_damage == damage_including_camo);
}

TEST_CASE("Test NormalDamageCalculator - camo - good effect from matching camo", "[require]") {
  auto unit_a = UnitFactory::standard(0, 0), unit_b = UnitFactory::standard(1, 1);
...
TEST_CASE("Test NormalDamageCalculator - camo - no effect from non-matching camo", "[require]") {
  auto unit_a = UnitFactory::standard(0, 0), unit_b = UnitFactory::standard(1, 1);
...
TEST_CASE("Test NormalDamageCalculator - ducking - no effect from not ducking", "[require]") {
  auto unit_a = UnitFactory::standard(0, 0), unit_b = UnitFactory::standard(1, 1);
  src::combat::Terrain terrain{0, 0, src::unit::CamoType::Normal}; // 0 cover, 0 max range, normal landscape
  std::dynamic_pointer_cast<src::unit::Unit>(unit_b)->duck(false);

  float theoretical_damage = Normal(unit_a, unit_b, terrain).calculate_damage_over_distance();
  float damage_including_ducking = Normal(unit_a, unit_b, terrain).calculate_damage_through_cover_and_ducking(theoretical_damage);

  REQUIRE(theoretical_damage == damage_including_ducking);
}

TEST_CASE("Test NormalDamageCalculator - ducking - good effect from ducking", "[require]") {
  auto unit_a = UnitFactory::standard(0, 0), unit_b = UnitFactory::standard(1, 1);
  src::combat::Terrain terrain{0, 0, src::unit::CamoType::GrassPlains}; // 0 cover, 0 max range, GrassPlains landscape
...
TEST_CASE("Test NormalDamageCalculator - ducking - no additional benefit from ducking if in cover", "[require]") {
  auto unit_a = UnitFactory::standard(0, 0), unit_b = UnitFactory::standard(1, 1);
  src::combat::Terrain terrain{1000, 0, src::unit::CamoType::GrassPlains}; // 1000 cover, 0 max range, GrassPlains landscape
  std::dynamic_pointer_cast<src::unit::Unit>(unit_b)->duck(true);

  float theoretical_damage = Normal(unit_a, unit_b, terrain).calculate_damage_over_distance();
  float damage_actually_from_cover = Normal(unit_a, unit_b, terrain).calculate_damage_through_cover_and_ducking(theoretical_damage);

  REQUIRE(src::combat::Terrain::_ducking_factor * theoretical_damage > damage_actually_from_cover);
}

TEST_CASE("Test NormalDamageCalculator - digging in - no effect from not being dug in", "[require]") {
  auto unit_a = UnitFactory::standard(0, 0), unit_b = UnitFactory::standard(1, 1);
  src::combat::Terrain terrain{0, 0, src::unit::CamoType::Normal}; // 0 cover, 0 max range, normal landscape
  std::dynamic_pointer_cast<src::unit::Unit>(unit_b)->set_dug_in(false);

  float theoretical_damage = Normal(unit_a, unit_b, terrain).calculate_damage_over_distance();
  float damage_including_dug_in = Normal(unit_a, unit_b, terrain).calculate_damage_through_cover_and_ducking(theoretical_damage);

  REQUIRE(theoretical_damage == damage_including_dug_in);
}

TEST_CASE("Test NormalDamageCalculator - digging in - being dug in counts as covered", "[require]") {
  auto unit_a = UnitFactory::standard(0, 0), unit_b = UnitFactory::standard(1, 1);
...
TEST_CASE("Test NormalDamageCalculator - cover - insufficient to hit uncovered (1/4)", "[require]") {
  // low damage scenario:
...
TEST_CASE("Test NormalDamageCalculator - cover - sufficient to hit uncovered (2/4)", "[require]") {
...
TEST_CASE("Test NormalDamageCalculator - cover - insufficient to hit all covered (3/4)", "[require]") {
...
TEST_CASE("Test NormalDamageCalculator - cover - sufficient to hit all covered (4/4)", "[require]") {
