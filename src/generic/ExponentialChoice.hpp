...
template<typename T> size_t exponential_choice(const std::vector<T>& v, float exponent) {

    // leave these function-internal variables static to reuse them if v.size() doesn't change between multiple executions
